...
const Header = ({ loader }: HeaderProps) => {
  ...
  return (
    ...
    >
      ...
      <div className={cn(styles.bar, "flex items-center justify-between w-full max-w-7xl mx-auto px-2 sm:px-4")}>
        <Link href="/" className="flex items-center justify-center shrink-0">
          <Button variant={"link"} className="text-sm sm:text-md px-1 sm:px-3 font-semibold truncate max-w-[140px] sm:max-w-none">
            {config.author}
          </Button>
        </Link>

        <div className="flex items-center gap-1 sm:gap-2 ml-auto mr-1 sm:mr-4 shrink-0">
          <Button
            variant="ghost"
            size="icon"
            onClick={toggleMute}
          ...
          >
            {isMuted ? <VolumeX className="w-[1.1rem] h-[1.1rem]" /> : <Volume2 className="w-[1.1rem] h-[1.1rem]" />}
          </Button>
        ...
        {config.githubUsername && config.githubRepo && (
          <GitHubStarsButton
            ...
            className="mr-2 sm:mr-4 hidden xs:inline-flex scale-90 sm:scale-100"
          />
        )}
        <Button
          variant={"ghost"}
